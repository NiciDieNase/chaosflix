...
//		updateCardBackgroundColor(cardView, false);
    ...
    public void onBindViewHolder(Presenter.ViewHolder viewHolder, Object item) {
		...
		if (item instanceof Conference) {
            ...
            loadImage(viewHolder.view.getContext(), conference.getLogoUrl(), mainImageView);
...
//			cardView.setContentText(android.text.TextUtils.join(", ",event.getPersons()));
            event.getThumbUrl();
            loadImage(viewHolder.view.getContext(), event.getThumbUrl(), mainImageView);
            ...
            room.getThumb();
            loadImage(viewHolder.view.getContext(), room.getThumb(), mainImageView);
        } else if (item instanceof SelectableContentItem) {
            SelectableContentItem sci = (SelectableContentItem) item;
            cardView.setTitleText(sci.getTitle());
            mainImageView.setBackgroundResource(R.drawable.focused_icon);
            mainImageView.getBackground().setAlpha(0);
            cardView.setOnFocusChangeListener((v, hasFocus) -> {
                if (hasFocus) {
                    ObjectAnimator.ofInt(mainImageView.getBackground(), "alpha", 0, 255).setDuration(200).start();
                } else {
                    ObjectAnimator.ofInt(mainImageView.getBackground(), "alpha", 255, 0).setDuration(200).start();
                }
            });
            Glide.with(viewHolder.view.getContext())
                    .load(((SelectableContentItem) item).getIcon())
                    .into(mainImageView);
    ...
    private void loadImage(Context context, String source, ImageView destination) {
